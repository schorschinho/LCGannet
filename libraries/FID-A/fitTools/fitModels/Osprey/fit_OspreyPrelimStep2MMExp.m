...
for ii = 1:nBasisFcts
    if ii <= nMets 
        J(:,4+ii) = J(:,4+ii) + conv(tempBasis.specs(:,ii), lineShapeNorm, 'same')*ampl(ii); 
    else  % No convolution is applied to the MM functions
        J(:,4+ii) = J(:,4+ii) + tempBasis.specs(:,ii)*ampl(ii);             
    end
end
...
for ii = 1:nBasisFcts
    if ii <= nMets 
        J(:,4+nBasisFcts+ii) = J(:,4+nBasisFcts+ii) + conv(tempBasis.specs(:,ii), lineShapeNorm, 'same')*ampl(ii);  
    else  % No convolution is applied to the MM functions
        J(:,4+nBasisFcts+ii) = J(:,4+nBasisFcts+ii) + tempBasis.specs(:,ii)*ampl(ii);             
    end
end

% derivative  wrt basis set  amplitudes 
for ii=1:nBasisFcts
        J(:,4+2*nBasisFcts+ii) = Acomp(:,ii);
end


% derivative wrt spline amplitudes
for ii=1:nSplines
    J(:,4+3*nBasisFcts+ii) = Bcomp(:,ii);
end
...
for ii=1:length(lineShape)
    ...
    J(:, 4+3*nBasisFcts+nSplines+ii) = J(:, 4+3*nBasisFcts+nSplines+ii) + A;
end
